...
void RW_IN_Init(in_state_t * in_state_p)
{
	...
	throttle_axis =
	    (int)ri.Cvar_Get("j_throttle", "3", CVAR_ARCHIVE)->value;
#endif
		...
		if (mouse_x || mouse_y) {
			if (!exponential_speedup->value) {
				...
				if (mouse_x > MOUSE_MIN || mouse_y > MOUSE_MIN
				    ...
				    || mouse_y < -MOUSE_MIN) {
					mouse_x =
					    (mouse_x * mouse_x * mouse_x) / 4;
					mouse_y =
					    (mouse_y * mouse_y * mouse_y) / 4;
...
#include "q2icon.xbm"
	...
	icon =
	    SDL_CreateRGBSurface(SDL_SWSURFACE, q2icon_width, q2icon_height, 8,
				 0, 0, 0, 0);
	if (icon == NULL)
		return;		/* oh well... */
...
*/
	...
	sdl_palettemode =
	    (vinfo->vfmt->BitsPerPixel ==
	     8) ? (SDL_PHYSPAL | SDL_LOGPAL) : SDL_LOGPAL;
	...
	if ((surface =
	     SDL_SetVideoMode(vid.width, vid.height, 8, flags)) == NULL) {
