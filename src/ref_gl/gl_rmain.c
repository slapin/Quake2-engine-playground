...
void R_DrawParticles(void)
{
	if (gl_ext_pointparameters->value && qglPointParameterfEXT) {
		...
		for (i = 0, p = r_newrefdef.particles;
		     i < r_newrefdef.num_particles; i++, p++) {
...
void R_Register(void)
{
	...
	gl_particle_att_b =
	    ri.Cvar_Get("gl_particle_att_b", "0.0", CVAR_ARCHIVE);
	gl_particle_att_c =
...
int R_Init(void *hinstance, void *hWnd)
{
	...
	if (!QGL_Init(gl_driver->string)) {
		QGL_Shutdown();
		ri.Con_Printf(PRINT_ALL,
			      "ref_gl::R_Init() - could not load \"%s\"\n",
			      gl_driver->string);
		return -1;
	}
	...
	strncpy(renderer_buffer, gl_config.renderer_string,
	...
	if (strstr(gl_config.extensions_string, "3DFX_set_global_palette")) {
		if (gl_ext_palettedtexture->value) {
			...
			ri.Con_Printf(PRINT_ALL,
				      "...ignoring 3DFX_set_global_palette\n");
		}
	} else {
		ri.Con_Printf(PRINT_ALL,
...
void R_SetPalette(const unsigned char *palette)
{
	...
	if (palette) {
		...
		for (i = 0; i < 256; i++) {
			...
			rp[i * 4 + 1] =
			    (LittleLong(d_8to24table[i]) >> 8) & 0xff;
			rp[i * 4 + 2] =
			    (LittleLong(d_8to24table[i]) >> 16) & 0xff;
			rp[i * 4 + 3] = 0xff;
		}
	}
