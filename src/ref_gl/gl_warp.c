...
void SubdividePolygon(int numverts, float *verts)
{
	...
	poly =
	    Hunk_Alloc(sizeof(glpoly_t) +
		       ((numverts - 4) + 2) * VERTEXSIZE * sizeof(float));
	...
	for (bp = fa->polys; bp; bp = bp->next) {
		...
		for (i = 0, v = p->verts[0]; i < p->numverts;
			...
			s = os +
			    r_turbsin[(int)
				      ((ot * 0.125 +
					r_newrefdef.time) * TURBSCALE) & 255];
#else
			s = os +
			    r_turbsin[Q_ftol(((ot * 0.125 + rdt) * TURBSCALE)) &
				      255];
#endif
			s += scroll;
			s *= (1.0 / 64);
...
#if !id386
			t = ot +
			    r_turbsin[(int)((os * 0.125 + rdt) * TURBSCALE) &
				      255];
#else
			t = ot +
			    r_turbsin[Q_ftol(((os * 0.125 + rdt) * TURBSCALE)) &
				      255];
#endif
			t *= (1.0 / 64);
