...
model_t *Mod_ForName(char *name, qboolean crash)
{
	...
	if (!buf) {
		if (crash)
			ri.Sys_Error(ERR_DROP, "Mod_NumForName: %s not found",
				     mod->name);
...
float RadiusFromBounds(vec3_t mins, vec3_t maxs)
{
	...
	for (i = 0; i < 3; i++) {
		corner[i] =
		    fabs(mins[i]) >
		    fabs(maxs[i]) ? fabs(mins[i]) : fabs(maxs[i]);
	}
		...
		if (out->
			...
			for (j = 0; j < out->nummarksurfaces; j++) {
				out->firstmarksurface[j]->flags |=
				    SURF_UNDERWATER;
				for (poly = out->firstmarksurface[j]->polys;
				     poly; poly = poly->next)
	...
	for (i = 0; i < mod->numsubmodels; i++) {
		...
		*starmod = *loadmodel;
		...
		if (starmod->firstnode >= loadmodel->numnodes)
			ri.Sys_Error(ERR_DROP,
				     "Inline model %i has bad firstnode", i);
...
void Mod_LoadAliasModel(model_t * mod, void *buffer)
{
	...
	if (pheader->num_xyz > MAX_VERTS)
		ri.Sys_Error(ERR_DROP, "model %s has too many vertices",
			     mod->name);

	if (pheader->num_st <= 0)
		ri.Sys_Error(ERR_DROP, "model %s has no st vertices",
			     mod->name);
...
//
	...
	for (i = 0; i < pheader->num_tris; i++) {
		for (j = 0; j < 3; j++) {
			pouttri[i].index_xyz[j] =
			    LittleShort(pintri[i].index_xyz[j]);
...
//
	for (i = 0; i < pheader->num_frames; i++) {
		...
		memcpy(poutframe->name, pinframe->name,
		       sizeof(poutframe->name));
		for (j = 0; j < 3; j++) {
			poutframe->scale[j] = LittleFloat(pinframe->scale[j]);
			poutframe->translate[j] =
			    LittleFloat(pinframe->translate[j]);
		}
	...
	for (i = 0; i < pheader->num_skins; i++) {
		mod->skins[i] =
		    GL_FindImage((char *)pheader + pheader->ofs_skins +
...
void Mod_LoadSpriteModel(model_t * mod, void *buffer)
{
	...
	for (i = 0; i < sprout->numframes; i++) {
		...
		sprout->frames[i].origin_x =
		    LittleLong(sprin->frames[i].origin_x);
		sprout->frames[i].origin_y =
		    LittleLong(sprin->frames[i].origin_y);
...
struct model_s *R_RegisterModel(char *name)
{
	...
	if (mod) {
		...
		if (mod->type == mod_sprite) {
			...
			for (i = 0; i < sprout->numframes; i++)
				mod->skins[i] =
				    GL_FindImage(sprout->frames[i].name,
						 it_sprite);
		} else if (mod->type == mod_alias) {
			pheader = (dmdl_t *) mod->extradata;
			for (i = 0; i < pheader->num_skins; i++)
				mod->skins[i] =
				    GL_FindImage((char *)pheader +
						 pheader->ofs_skins +
						 i * MAX_SKINNAME, it_skin);
//PGM
