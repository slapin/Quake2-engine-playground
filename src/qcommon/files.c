...
int FS_FOpenFile(char *filename, FILE ** file)
{
	...
	for (link = fs_links; link; link = link->next) {
		if (!strncmp(filename, link->from, link->fromlength)) {
			Com_sprintf(netpath, sizeof(netpath), "%s%s", link->to,
...
//
	for (search = fs_searchpaths; search; search = search->next) {
		...
		if (search->pack) {
					...
					*file = fopen(pak->filename, "rb");
					if (!*file)
						Com_Error(ERR_FATAL,
							  "Couldn't reopen %s",
							  pak->filename);
					fseek(*file, pak->files[i].filepos,
					      SEEK_SET);
			...
			Com_sprintf(netpath, sizeof(netpath), "%s/%s",
...
int FS_FOpenFile(char *filename, FILE ** file)
{
	...
	if (!strcmp(filename, "config.cfg")
	    || !strncmp(filename, "players/", 8)) {
		Com_sprintf(netpath, sizeof(netpath), "%s/%s", FS_Gamedir(),
			    filename);
...
void FS_AddHomeAsGameDirectory(char *dir)
{
	...
	if (homedir) {
		int len =
		    snprintf(gdir, sizeof(gdir), "%s/.quake2/%s/", homedir,
			     dir);
...
void FS_ExecAutoexec(void)
{
	...
	for (s = fs_searchpaths; s != end; s = s->next) {
		...
		if (Sys_FindFirst
		    (name, 0, SFF_SUBDIR | SFF_HIDDEN | SFF_SYSTEM)) {
...
void FS_Dir_f(void)
{
	...
	while ((path = FS_NextPath(path)) != NULL) {
		...
		Com_sprintf(findname, sizeof(findname), "%s/%s", path,
			    wildcard);
