...
void AI_SetSightClient(void)
{
	...
	while (1) {
		...
		ent = &g_edicts[check];
		if (ent->inuse
		    && ent->health > 0 && !(ent->flags & FL_NOTARGET)) {
...
void ai_stand(edict_t * self, float dist)
{
	...
	if (!(self->spawnflags & 1) && (self->monsterinfo.idle)
		...
		if (self->monsterinfo.idle_time) {
			...
			self->monsterinfo.idle_time =
			    level.time + 15 + random() * 15;
		} else {
			self->monsterinfo.idle_time =
			    level.time + random() * 15;
		}
	}
}
...
// this is where we would check invisibility
		...
		if (r == RANGE_NEAR) {
			if (client->show_hostile < level.time
			    && !infront(self, client)) {
				return false;
			}
		} else if (r == RANGE_MID) {
...
//              dprint("lost sight of player, last seen at "); dprint(vtos(self.last_sighting)); dprint("\n");
		self->monsterinfo.aiflags |=
		    (AI_LOST_SIGHT | AI_PURSUIT_LAST_SEEN);
		...
		if (tr.fraction < 1) {
			...
			if (left >= center && left > right) {
				if (left < 1) {
					...
					G_ProjectSource(self->s.origin, v,
							v_forward, v_right,
							left_target);
