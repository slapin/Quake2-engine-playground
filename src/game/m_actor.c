...
void actorMachineGun(edict_t * self)
{
	...
	G_ProjectSource(self->s.origin,
			monster_flash_offset[MZ2_ACTOR_MACHINEGUN_1], forward,
			right, start);
	if (self->enemy) {
	...
	monster_fire_bullet(self, start, forward, 3, 4, DEFAULT_BULLET_HSPREAD,
			    DEFAULT_BULLET_VSPREAD, MZ2_ACTOR_MACHINEGUN_1);
}
...
void actor_die(edict_t * self, edict_t * inflictor, edict_t * attacker,
...
void target_actor_touch(edict_t * self, edict_t * other, cplane_t * plane,
			csurface_t * surf)
{
	vec3_t v;
	...
	if (self->spawnflags & 1)	//jump
	{
		...
		if (other->groundentity) {
			...
			gi.sound(other, CHAN_VOICE,
				 gi.soundindex("player/male/jump1.wav"), 1,
				 ATTN_NORM, 0);
		}
	}
