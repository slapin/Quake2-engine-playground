...
void hover_fire_blaster(edict_t * self)
{
	...
	G_ProjectSource(self->s.origin,
			monster_flash_offset[MZ2_HOVER_BLASTER_1], forward,
			right, start);
	...
	monster_fire_blaster(self, start, dir, 1, 1000, MZ2_HOVER_BLASTER_1,
			     effect);
}
	...
	if (self->health <= self->gib_health) {
		...
		for (n = 0; n < 2; n++)
			ThrowGib(self, "models/objects/gibs/sm_meat/tris.md2",
				 damage, GIB_ORGANIC);
		ThrowHead(self, "models/objects/gibs/sm_meat/tris.md2", damage,
			  GIB_ORGANIC);
