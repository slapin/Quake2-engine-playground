...
void mutant_jump_touch(edict_t * self, edict_t * other, cplane_t * plane,
		       csurface_t * surf)
{
	if (self->health <= 0) {
		self->touch = NULL;
...
qboolean mutant_check_jump(edict_t * self)
{
	...
	if (self->absmin[2] >
	    (self->enemy->absmin[2] + 0.75 * self->enemy->size[2]))
		return false;

	if (self->absmax[2] <
	    (self->enemy->absmin[2] + 0.25 * self->enemy->size[2]))
		return false;
...
void mutant_die(edict_t * self, edict_t * inflictor, edict_t * attacker,
