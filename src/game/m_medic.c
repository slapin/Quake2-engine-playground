...
void medic_fire_blaster(edict_t * self)
{
	...
	G_ProjectSource(self->s.origin,
			monster_flash_offset[MZ2_MEDIC_BLASTER_1], forward,
			right, start);
...
void medic_die(edict_t * self, edict_t * inflictor, edict_t * attacker,
...
void medic_cable_attack(edict_t * self)
{
	...
	tr = gi.trace(start, NULL, NULL, self->enemy->s.origin, self,
		      MASK_SHOT);
	if (tr.fraction != 1.0 && tr.ent != self->enemy)
		return;

	if (self->s.frame == FRAME_attack43) {
		gi.sound(self->enemy, CHAN_AUTO, sound_hook_hit, 1, ATTN_NORM,
			 0);
		...
		if (self->s.frame == FRAME_attack44)
			gi.sound(self, CHAN_WEAPON, sound_hook_heal, 1,
				 ATTN_NORM, 0);
	}
