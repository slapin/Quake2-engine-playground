...
mmove_t makron_move_stand =
    { FRAME_stand201, FRAME_stand260, makron_frames_stand, NULL };
...
mmove_t makron_move_run =
    { FRAME_walk204, FRAME_walk213, makron_frames_run, NULL };
...
mmove_t makron_move_pain6 =
    { FRAME_pain601, FRAME_pain627, makron_frames_pain6, makron_run };
...
mmove_t makron_move_pain5 =
    { FRAME_pain501, FRAME_pain504, makron_frames_pain5, makron_run };
...
mmove_t makron_move_pain4 =
    { FRAME_pain401, FRAME_pain404, makron_frames_pain4, makron_run };
...
mmove_t makron_move_death2 =
    { FRAME_death201, FRAME_death295, makron_frames_death2, makron_dead };
...
mmove_t makron_move_death3 =
    { FRAME_death301, FRAME_death320, makron_frames_death3, NULL };
...
mmove_t makron_move_sight =
    { FRAME_active01, FRAME_active13, makron_frames_sight, makron_run };
...
mmove_t makron_move_attack3 =
    { FRAME_attak301, FRAME_attak308, makron_frames_attack3, makron_run };
...
mmove_t makron_move_attack4 =
    { FRAME_attak401, FRAME_attak426, makron_frames_attack4, makron_run };
...
mmove_t makron_move_attack5 =
    { FRAME_attak501, FRAME_attak516, makron_frames_attack5, makron_run };
...
void MakronRailgun(edict_t * self)
{
	...
	G_ProjectSource(self->s.origin,
			monster_flash_offset[MZ2_MAKRON_RAILGUN_1], forward,
			right, start);
...
void MakronHyperblaster(edict_t * self)
{
	...
	if (self->s.frame <= FRAME_attak413)
		dir[1] =
		    self->s.angles[1] - 10 * (self->s.frame - FRAME_attak413);
	else
		dir[1] =
		    self->s.angles[1] + 10 * (self->s.frame - FRAME_attak421);
	dir[2] = 0;
...
void makron_pain(edict_t * self, edict_t * other, float kick, int damage)
{
	...
	if (damage <= 40) {
		...
		if (damage <= 150) {
			if (random() <= 0.45) {
				gi.sound(self, CHAN_VOICE, sound_pain6, 1,
					 ATTN_NONE, 0);
...
void makron_die(edict_t * self, edict_t * inflictor, edict_t * attacker,
	...
	if (self->health <= self->gib_health) {
		...
		for (n = 0; n < 4; n++)
			ThrowGib(self, "models/objects/gibs/sm_metal/tris.md2",
				 damage, GIB_METALLIC);
		ThrowHead(self, "models/objects/gibs/gear/tris.md2", damage,
			  GIB_METALLIC);
