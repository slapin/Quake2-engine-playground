...
void jorg_pain(edict_t * self, edict_t * other, float kick, int damage)
{
	...
	if (damage <= 50) {
		...
		if (random() <= 0.3) {
			gi.sound(self, CHAN_VOICE, sound_pain3, 1, ATTN_NORM,
				 0);
...
void jorg_firebullet_right(edict_t * self)
{
	...
	G_ProjectSource(self->s.origin,
			monster_flash_offset[MZ2_JORG_MACHINEGUN_R1], forward,
			right, start);
...
void jorg_firebullet_left(edict_t * self)
{
	...
	AngleVectors(self->s.angles, forward, right, NULL);
	G_ProjectSource(self->s.origin,
			monster_flash_offset[MZ2_JORG_MACHINEGUN_L1], forward,
			right, start);
...
void jorg_die(edict_t * self, edict_t * inflictor, edict_t * attacker,
...
void SP_monster_jorg(edict_t * self)
{
	...
	self->s.modelindex2 =
	    gi.modelindex("models/monsters/boss3/jorg/tris.md2");
