...
void TankBlaster(edict_t * self)
{
	...
	monster_fire_blaster(self, start, dir, 30, 800, flash_number,
			     EF_BLASTER);
}
...
void TankMachineGun(edict_t * self)
{
	...
	if (self->s.frame <= FRAME_attak415)
		dir[1] =
		    self->s.angles[1] - 8 * (self->s.frame - FRAME_attak411);
	else
		dir[1] =
		    self->s.angles[1] + 8 * (self->s.frame - FRAME_attak419);
	dir[2] = 0;
...
void tank_die(edict_t * self, edict_t * inflictor, edict_t * attacker,
