...
void NetadrToSockadr(netadr_t * a, struct sockaddr_storage *s)
{
	...
	case NA_MULTICAST6:
		...
		if (inet_pton(AF_INET6, QUAKE2MCAST, &s6->sin6_addr.s6_addr) !=
		    1) {
...
char *NET_BaseAdrToString(netadr_t a)
{
	...
	case NA_IP:
	case NA_LOOPBACK:
		Com_sprintf(s, sizeof(s), "%i.%i.%i.%i", a.ip[0], a.ip[1],
			    a.ip[2], a.ip[3]);
		break;
	case NA_BROADCAST:
...
#else
		if (getnameinfo
		    ((struct sockaddr *)&ss, sizeof(ss), s, sizeof(s), NULL, 0,
		     NI_NUMERICHOST))
#endif
...
qboolean NET_StringToSockaddr(char *s, struct sockaddr_storage * sadr)
{
	...
	if (*addrs == '[') {
		...
		if (!*space) {
			Com_Printf
			    ("NET_StringToSockaddr: invalid IPv6 address %s\n",
			     s);
			return 0;
		}
		*space++ = '\0';
		       ...
		       sizebuf_t * net_message)
{
	...
	for (protocol = 0; protocol < 3; protocol++) {
		...
		fromlen = sizeof(from);
		ret =
		    recvfrom(net_socket, net_message->data,
	...
	if (addr.ss_family == AF_INET6) {
		...
		if (IN6_IS_ADDR_MULTICAST(&s6->sin6_addr)) {
			...
			if (multicast_interface != NULL) {
				...
				if (error) {
					Com_Printf
					    ("NET_SendPacket: getnameinfo: %s",
					     gai_strerror(error));
					return;
				}

				Com_sprintf(mcast_addr, sizeof(mcast_addr),
					    "%s%%%s", tmp, multicast_interface);
				Com_sprintf(mcast_port, sizeof(mcast_port),
					    "%d", ntohs(s6->sin6_port));
				memset(&hints, 0, sizeof(hints));
				...
				error =
				    getaddrinfo(mcast_addr, mcast_port, &hints,
						&res);
				if (error) {
					Com_Printf
					    ("NET_SendPacket: getaddrinfo: %s",
					     gai_strerror(error));
					return;
				}
				...
				Com_Printf
				    ("NET_SendPacket: IPv6 multicast destination but +set multicast not specified: %s\n",
				     inet_ntop(AF_INET6, &s6->sin6_addr, tmp,
					       sizeof(tmp)));
				return;
			}
...
void NET_OpenIP(void)
{
	...
	if (!ip6_sockets[NS_SERVER])
		ip6_sockets[NS_SERVER] =
		    NET_Socket(ip->string, port->value, NS_SERVER, AF_INET6);
	if (!ip6_sockets[NS_CLIENT])
		ip6_sockets[NS_CLIENT] =
		    NET_Socket(ip->string, PORT_ANY, NS_CLIENT, AF_INET6);
	if (!ip_sockets[NS_SERVER])
		ip_sockets[NS_SERVER] =
		    NET_Socket(ip->string, port->value, NS_SERVER, AF_INET);
	if (!ip_sockets[NS_CLIENT])
		ip_sockets[NS_CLIENT] =
		    NET_Socket(ip->string, PORT_ANY, NS_CLIENT, AF_INET);
}
...
/*                 Com_Printf("NET_Socket: getaddrinfo: %s\n", gai_strerror (Error)); */
	...
	for (ai = res; ai != NULL; ai = ai->ai_next) {
		...
		if (family == AF_INET6)
			if (setsockopt
			    (newsocket, IPPROTO_IPV6, IPV6_BINDV6ONLY, &dummy,
			     sizeof(dummy)) < 0) {
				Com_Printf
				    ("NET_Socket: setsockopt IPV6_BINDV6ONLY: %s\n",
				     strerror(errno));
			}
#endif
		if (family == AF_INET) {
			...
			if (setsockopt
			    (newsocket, SOL_SOCKET, SO_BROADCAST, (char *)&i,
			     sizeof(i)) == -1) {
				Com_Printf
	...
	case AF_INET6:
		...
		if (multicast_interface != NULL) {
			...
			if (setsockopt
			    (newsocket, IPPROTO_IPV6, IPV6_MULTICAST_IF,
			...
			if (type == NS_SERVER) {
				if (inet_pton
				    (AF_INET6, QUAKE2MCAST,
				     &mreq.ipv6mr_multiaddr.s6_addr) != 1) {
					Com_Printf("NET_Socket: inet_pton: %s",
						   strerror(errno));
				}
				if (setsockopt
				    (newsocket, IPPROTO_IPV6, IPV6_JOIN_GROUP,
				     &mreq, sizeof(mreq)) < 0) {
					Com_Printf
					    ("NET_Socket: IPV6_JOIN_GROUP: %s",
					     strerror(errno));
				}
			}
		}
...
void NET_Sleep(int msec)
{
	...
	if ((!ip_sockets[NS_SERVER] && !ip6_sockets[NS_SERVER])
	...
	select(MAX(ip_sockets[NS_SERVER], ip6_sockets[NS_SERVER]) + 1, &fdset,
	       NULL, NULL, &timeout);
}
