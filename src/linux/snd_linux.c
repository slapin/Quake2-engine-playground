...
// open /dev/dsp, confirm capability to mmap, and get size of dma buffer
	...
	if (!(caps & DSP_CAP_TRIGGER) || !(caps & DSP_CAP_MMAP)) {
		Com_Printf
		    ("SNDDMA_Init: Sorry, but your soundcard doesn't support trigger or mmap. (%08x)\n",
		     caps);
		close(audio_fd);
		audio_fd = -1;
		return 0;
...
// set sample bits & speed
	...
	if (dma.samplebits == 16) {
		...
		if (rc < 0) {
			perror(snddevice->string);
			Com_Printf
			    ("SNDDMA_Init: Could not support 16-bit data.  Try 8-bit.\n");
			close(audio_fd);
			audio_fd = -1;
			return 0;
		}
		...
		if (rc < 0) {
			perror(snddevice->string);
			Com_Printf
			    ("SNDDMA_Init: Could not support 8-bit data.\n");
			close(audio_fd);
			audio_fd = -1;
			return 0;
		}
		...
		perror(snddevice->string);
		Com_Printf("SNDDMA_Init: %d-bit sound not supported.",
			   dma.samplebits);
		close(audio_fd);
		audio_fd = -1;
		return 0;
	...
	if (rc < 0) {
		perror(snddevice->string);
		Com_Printf("SNDDMA_Init: Could not set %s to stereo=%d.",
			   snddevice->string, dma.channels);
		close(audio_fd);
		audio_fd = -1;
		return 0;
	}
	...
	if (rc < 0) {
		perror(snddevice->string);
		Com_Printf("SNDDMA_Init: Could not set %s speed to %d.",
			   snddevice->string, dma.speed);
		close(audio_fd);
		audio_fd = -1;
		return 0;
	}
