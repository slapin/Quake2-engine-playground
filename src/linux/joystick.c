			...
			cvar_t * lookstrafe, cvar_t * m_pitch)
{
	...
	if (joy_avail) {
		...
		if ((joy_advanced->value == 0.0) && mlooking) {
			...
			cmd->forwardmove +=
			    jforward * joy_forwardsensitivity->value * speed *
			    cl_forwardspeed->value;
		}

		if (fabs(jside) > joy_sidethreshold->value) {
			cmd->sidemove +=
			    (jside * joy_sidesensitivity->value) * speed *
			    cl_sidespeed->value;
		}

		if (fabs(jup) > joy_upthreshold->value) {
			cmd->upmove +=
			    (jup * joy_upsensitivity->value) * speed *
			    cl_upspeed->value;
		}
		...
		if (((*in_state->in_strafe_state) & 1)
			...
			if (fabs(jturn) > joy_sidethreshold->value) {
				cmd->sidemove -=
				    (jturn * joy_sidesensitivity->value) *
				    speed * cl_sidespeed->value;
			}
		} else {
			// user wants turn control to be turn control
			if (fabs(jturn) > joy_yawthreshold->value) {
				in_state->viewangles[YAW] +=
				    (jturn * joy_yawsensitivity->value) *
		...
		if (fabs(jlook) > joy_pitchthreshold->value) {
			...
			in_state->viewangles[PITCH] +=
			    (jlook * joy_pitchsensitivity->value) * aspeed *
			    cl_pitchspeed->value;
		}
		//}
