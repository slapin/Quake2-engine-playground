...
//GLX Functions
static XVisualInfo *(*qglXChooseVisual) (Display * dpy, int screen,
					 int *attribList);
static GLXContext(*qglXCreateContext) (Display * dpy, XVisualInfo * vis,
				       GLXContext shareList, Bool direct);
static void (*qglXDestroyContext) (Display * dpy, GLXContext ctx);
static Bool(*qglXMakeCurrent) (Display * dpy, GLXDrawable drawable,
			       GLXContext ctx);
static void (*qglXCopyContext) (Display * dpy, GLXContext src, GLXContext dst,
				GLuint mask);
	...
	if (err) {
		...
		case GLOB_NOSPACE:
			ri.Con_Printf(PRINT_ALL,
				      "Error, out of memory while looking for joysticks\n");
			break;
		case GLOB_NOMATCH:
		...
		default:
			ri.Con_Printf(PRINT_ALL,
				      "Error #%d while looking for joysticks\n",
				      err);
		}
		return false;
	}
...
// inviso cursor
	...
	XGrabPointer(dpy, win,
		     True,
		     0, GrabModeAsync, GrabModeAsync, win, None, CurrentTime);
	...
	if (in_dgamouse->value) {
		...
		XWarpPointer(dpy, None, win, 0, 0, 0, 0, vid.width / 2,
			     vid.height / 2);
...
void ResetSharedFrameBuffers(void)
{
	...
	for (frm = 0; frm < 2; frm++) {
		...
		key = random();
		x_shminfo[frm].shmid =
		    shmget((key_t) key, size, IPC_CREAT | 0777);
	...
	while (XPending(dpy)) {
		...
		case KeyPress:
			...
			if (in_state && in_state->Key_Event_fp)
				in_state->Key_Event_fp(XLateKey(&event.xkey),
						       true);
			break;
		case KeyRelease:
		...
		case CreateNotify:
			ri.Cvar_Set("vid_xpos",
				    va("%d", event.xcreatewindow.x));
			ri.Cvar_Set("vid_ypos",
				    va("%d", event.xcreatewindow.y));
...
// create the main window
		win =
		    XCreateWindow(dpy, root, (int)vid_xpos->value,
				  (int)vid_ypos->value, vid.width, vid.height,
...
// if the window changes dimension, skip this frame
	...
	if (doShm) {
		if (x_visinfo->depth == 16)
			st2_fixup(x_framebuffer[current_framebuffer], 0, 0,
				  vid.width, vid.height);
		else if (x_visinfo->depth == 24)
			st3_fixup(x_framebuffer[current_framebuffer], 0, 0,
				  vid.width, vid.height);
		if (!XShmPutImage
		    (dpy, win, x_gc, x_framebuffer[current_framebuffer], 0, 0,
...
void SWimp_SetPalette(const unsigned char *palette)
{
	...
	for (i = 0; i < 256; i++) {
#ifdef REDBLUE
		int tmp =
		    (30 * palette[i * 4] + 59 * palette[i * 4 + 1] +
		     11 * palette[i * 4 + 2]) / 100;
		...
		st2d_8to16table[i] = xlib_rgb16(palette[i * 4],
						...
						palette[i * 4 + 2]);
		st2d_8to24table[i] =
		    xlib_rgb24(palette[i * 4], palette[i * 4 + 1],
			       palette[i * 4 + 2]);
	}
...
int GLimp_SetMode(int *pwidth, int *pheight, int mode, qboolean fullscreen)
{
	...
	if (fullscreen)
		ri.Con_Printf(PRINT_ALL, "...setting fullscreen mode %d:",
			      mode);
	else
		ri.Con_Printf(PRINT_ALL, "...setting mode %d:", mode);
	...
	if (vidmode_ext) {
		...
		XF86VidModeGetAllModeLines(dpy, scrnum, &num_vidmodes,
					   &vidmodes);
...
#include "q2icon.xbm"
		...
		icon_pixmap =
		    XCreatePixmapFromBitmapData(dpy, win, (char *)q2icon_bits,
						q2icon_width, q2icon_height, fg,
						bg, visinfo->depth);
		for (i = 0; i < sizeof(q2icon_bits); i++)
			q2icon_bits[i] = ~q2icon_bits[i];
		icon_mask =
		    XCreatePixmapFromBitmapData(dpy, win, (char *)q2icon_bits,
						q2icon_width, q2icon_height, bg,
						fg, visinfo->depth);
...
void PlatformJoyCommands(int *axis_vals, int *axis_map)
{
	...
	while (read(joy_fd, &e, sizeof(struct js_event)) != -1) {
		if (JS_EVENT_BUTTON & e.type) {
			...
			if (e.value) {
				in_state->Key_Event_fp(key_index + e.number,
						       true);
			} else {
				in_state->Key_Event_fp(key_index + e.number,
						       false);
