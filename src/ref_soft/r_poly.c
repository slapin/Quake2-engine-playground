...
void R_DrawSpanletTurbulentStipple33(void)
{
	...
	if (s_spanletvars.v & 1) {
		...
		if (s_spanletvars.spancount == AFFINE_SPANLET_SIZE)
			s_spanletvars.izi +=
			    s_spanletvars.izistep << AFFINE_SPANLET_SIZE_BITS;
		else
			s_spanletvars.izi +=
			    s_spanletvars.izistep * s_spanletvars.izistep;
		...
		while (s_spanletvars.spancount > 0) {
			sturb =
			    ((s_spanletvars.s +
			      r_turb_turb[(s_spanletvars.t >> 16) & (CYCLE -
								     1)]) >> 16)
			    & 63;
			tturb =
			    ((s_spanletvars.t +
			      r_turb_turb[(s_spanletvars.s >> 16) & (CYCLE -
								     1)]) >> 16)
			    & 63;
...
// we count on FP exceptions being turned off to avoid range problems
	...
	do {
		s_spanletvars.pdest =
		    (byte *) d_viewbuffer +
		    (d_scantable[pspan->v] /*r_screenwidth * pspan->v */ ) +
		    pspan->u;
		s_spanletvars.pz =
		    d_pzbuffer + (d_zwidth * pspan->v) + pspan->u;
		...
		do {
			...
			if (count) {
				...
				// texture
				spancountminus1 =
				    (float)(s_spanletvars.spancount - 1);
	...
	for (i = 0; i < 4; i++) {
		...
		if (nump > MAXWORKINGVERTS)
			ri.Sys_Error(ERR_DROP,
				     "R_ClipAndDrawPoly: too many points: %d",
				     nump);
	}
...
void R_PolygonCalculateGradients(void)
{
	...
	sadjust =
	    (fixed16_t) ((DotProduct
			  (r_polydesc.viewer_position,
			   r_polydesc.vright) + r_polydesc.s_offset) * 0x10000);
	tadjust =
	    (fixed16_t) ((DotProduct(r_polydesc.viewer_position, r_polydesc.vup)
			  + r_polydesc.t_offset) * 0x10000);
