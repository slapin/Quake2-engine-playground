...
the complex cases add new polys on most lines,
    ...
    try to get better coherence ? low depth complexity-- 1 to 3 or so have a
    sentinal at both ends ?
#endif
 edge_t * auxedges;
...
void D_CalcGradients(msurface_t * pface)
{
	...
	if (pface->texinfo->flags & SURF_FLOWING) {
		if (pface->texinfo->flags & SURF_WARP)
			sadjust +=
			    0x10000 * (-128 *
				       ((r_newrefdef.time * 0.25) -
					(int)(r_newrefdef.time * 0.25)));
		else
			sadjust +=
			    0x10000 * (-128 *
				       ((r_newrefdef.time * 0.77) -
...
#if 1
	miplevel =
	    D_MipLevelForScale(s->nearzi * scale_for_mip *
