...
void D_ViewChanged(void)
{
	...
	if (r_newrefdef.rdflags & RDF_NOWORLDMODEL) {
		memset(d_pzbuffer, 0xff,
		       vid.width * vid.height * sizeof(d_pzbuffer[0]));
		Draw_Fill(r_newrefdef.x, r_newrefdef.y, r_newrefdef.width,
			  r_newrefdef.height, (int)sw_clearcolor->value & 0xff);
	}
...
__declspec(naked)
void TransformVector(vec3_t vin, vec3_t vout)
{
...
void R_ViewChanged(vrect_t * vr)
{
	...
	r_refdef.horizontalFieldOfView =
	    2 * tan((float)r_newrefdef.fov_x / 360 * M_PI);;
...
// current viewleaf
	...
	if (r_dowarp) {		// warp into off screen buffer
		vrect.x = 0;
		vrect.y = 0;
		vrect.width =
		    r_newrefdef.width <
		    WARP_WIDTH ? r_newrefdef.width : WARP_WIDTH;
		vrect.height =
		    r_newrefdef.height <
		    WARP_HEIGHT ? r_newrefdef.height : WARP_HEIGHT;
...
void R_ScreenShot_f(void)
{
	...
	Com_sprintf(checkname, sizeof(checkname), "%s/scrnshot",
		    ri.FS_Gamedir());
	Sys_Mkdir(checkname);
...
// 
	...
	if (i == 100) {
		ri.Con_Printf(PRINT_ALL,
			      "R_ScreenShot_f: Couldn't create a PCX");
		return;
	}
