...
void R_DrawBEntitiesOnList(void)
{
	...
	for (i = 0; i < r_newrefdef.num_entities; i++) {
		...
		if (topnode->contents == CONTENTS_NODE) {
			...
			// drawing order
			R_DrawSubmodelPolygons(currentmodel, clipflags,
					       topnode);
		}
...
void R_CalcPalette(void)
{
	...
	if (alpha <= 0) {
		if (modified) {	// set back to default
			modified = false;
			R_GammaCorrectAndSetPalette((const unsigned char *)
						    d_8to24table);
			return;
		}
		return;
...
void R_SetLightLevel(void)
{
	...
	if ((r_newrefdef.rdflags & RDF_NOWORLDMODEL) || (!r_drawentities->value)
	    || (!currententity)) {
...
void R_BeginFrame(float camera_separation)
{
	...
	while (sw_mode->modified || vid_fullscreen->modified) {
		 ...
		 */
		if ((err =
		     SWimp_SetMode(&vid.width, &vid.height, sw_mode->value,
			...
			if (err == rserr_invalid_mode) {
				...
				ri.Con_Printf(PRINT_ALL,
					      "ref_soft::R_BeginFrame() - fullscreen unavailable in this mode\n");
				sw_state.prev_mode = sw_mode->value;
//                              vid_fullscreen->modified = false;
//                              sw_mode->modified = false;
			} else {
				ri.Sys_Error(ERR_FATAL,
					     "ref_soft::R_BeginFrame() - catastrophic mode change failure\n");
			}
		}
	}
}
