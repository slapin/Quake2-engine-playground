...
void Action_Draw(menuaction_s * a)
{
	if (a->generic.flags & QMF_LEFT_JUSTIFY) {
		if (a->generic.flags & QMF_GRAYED)
			Menu_DrawStringDark(a->generic.x +
					    a->generic.parent->x +
					    LCOLUMN_OFFSET,
		...
		if (a->generic.flags & QMF_GRAYED)
			Menu_DrawStringR2LDark(a->generic.x +
					       a->generic.parent->x +
					       LCOLUMN_OFFSET,
					       a->generic.y +
					       a->generic.parent->y,
...
void Menu_Draw(menuframework_s * menu)
{
	...
	if (item && item->cursordraw) {
		...
		if (item->flags & QMF_LEFT_JUSTIFY) {
			...
			Draw_Char(menu->x + item->cursor_offset,
				  menu->y + item->y,
				  12 + ((int)(Sys_Milliseconds() / 250) & 1));
		}
	}
...
int Menu_TallySlots(menuframework_s * menu)
{
	...
	for (i = 0; i < menu->nitems; i++) {
		if (((menucommon_s *) menu->items[i])->type == MTYPE_LIST) {
			int nitems = 0;
			const char **n =
			    ((menulist_s *) menu->items[i])->itemnames;
...
void Slider_Draw(menuslider_s * s)
{
	...
	Draw_Char(RCOLUMN_OFFSET + s->generic.x + i * 8 + s->generic.parent->x +
		  8, s->generic.y + s->generic.parent->y, 130);
	Draw_Char((int)
		  (8 + RCOLUMN_OFFSET + s->generic.parent->x + s->generic.x +
...
void SpinControl_Draw(menulist_s * s)
{
	...
	if (!strchr(s->itemnames[s->curvalue], '\n')) {
		...
		*strchr(buffer, '\n') = 0;
		...
		Menu_DrawString(RCOLUMN_OFFSET + s->generic.x +
				s->generic.parent->x,
				s->generic.y + s->generic.parent->y + 10,
				buffer);
	}
}
