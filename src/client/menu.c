...
void M_DrawCursor(int x, int y, int f)
{
	...
	if (!cached) {
		...
		for (i = 0; i < NUM_CURSOR_FRAMES; i++) {
			Com_sprintf(cursorname, sizeof(cursorname),
				    "m_cursor%d", i);
...
static void KeyCursorDrawFunc(menuframework_s * menu)
{
	...
	if (bind_grab)
		...
		re.DrawChar(menu->x, menu->y + menu->cursor * 9,
			    12 + ((int)(Sys_Milliseconds() / 250) & 1), 1);
#else
	if (bind_grab)
		re.DrawChar(menu->x, menu->y + menu->cursor * 9, '=');
	else
...
static void ControlsSetMenuItemValues(void)
{
	s_options_sfxvolume_slider.curvalue =
	    Cvar_VariableValue("s_volume") * 10;
...
void Options_MenuInit(void)
{
	...
	s_options_customize_options_action.generic.callback =
	...
	Menu_AddItem(&s_options_menu,
		     (void *)&s_options_customize_options_action);
...
void M_Credits_MenuDraw(void)
{
	...
	for (i = 0, y =
		...
		for (j = 0; credits[i][j + stringoffset]; j++) {
			...
			x = (viddef.width - strlen(credits[i]) * 8 -
			     stringoffset * 8) / 2 + (j + stringoffset) * 8;

			if (bold)
#ifdef QMAX
				re.DrawChar(x, y,
					    credits[i][j + stringoffset] + 128,
					    1);
#else
				re.DrawChar(x, y,
					    credits[i][j + stringoffset] + 128);
...
void M_AddToServerList(netadr_t adr, char *info)
{
	...
	Com_sprintf(local_server_names[m_num_servers],
		    sizeof(local_server_names[0]) - 1, "%s %s", info,
		    NET_AdrToString(adr));
#else
...
void JoinServer_MenuInit(void)
{
	...
	for (i = 0; i < MAX_LOCAL_SERVERS; i++) {
		...
		s_joinserver_server_actions[i].generic.callback =
		    JoinServerFunc;
	...
	for (i = 0; i < 8; i++)
		Menu_AddItem(&s_joinserver_menu,
			     &s_joinserver_server_actions[i]);
...
void RulesChangeFunc(void *self)
{
	...
	{
		s_maxclients_field.generic.statusbar =
		    "4 maximum for cooperative";
...
//PGM
	...
	if (Cvar_VariableValue("maxclients") == 1)
		...
		strcpy(s_maxclients_field.buffer,
		       Cvar_VariableString("maxclients"));
...
//ROGUE
	if (Developer_searchpath(2) == 2) {
		...
		s_stack_double_box.curvalue =
		    (dmflags & DF_NO_STACK_DOUBLE) != 0;
	...
	for (i = 0; i < npms; i++) {
		...
		for (k = 0; k < npcxfiles - 1; k++) {
			if (!strstr(pcxnames[k], "_i.pcx")) {
				if (IconOfSkinExists
				    (pcxnames[k], pcxnames, npcxfiles - 1)) {
					nskins++;
				}
			}
		}
...
void PlayerConfig_MenuDraw(void)
{
	...
	if (s_pmi[s_player_model_box.curvalue].skindisplaynames) {
		...
		M_DrawTextBox((refdef.x) * (320.0F / viddef.width) - 8,
			      (viddef.height / 2) * (240.0F / viddef.height) -
			      77, refdef.width / 8, refdef.height / 8);
		refdef.height += 4;
