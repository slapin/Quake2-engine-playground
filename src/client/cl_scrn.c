...
void SCR_DrawConsole(void)
{
	...
	if (cls.state != ca_active || !cl.refresh_prepped) {	// connected, but can't render
		Con_DrawConsole(0.5);
		re.DrawFill(0, viddef.height / 2, viddef.width,
			    viddef.height / 2, 0);
		return;
	}
...
void SCR_TouchPics(void)
{
	...
	if (crosshair->value) {
		...
		Com_sprintf(crosshair_pic, sizeof(crosshair_pic), "ch%i",
			    (int)(crosshair->value));
		re.DrawGetPicSize(&crosshair_width, &crosshair_height,
				  crosshair_pic);
		if (!crosshair_width)
			crosshair_pic[0] = 0;
...
void SCR_ExecuteLayoutString(char *s)
{
	...
	while (s) {
		...
		if (!strcmp(token, "ctf")) {	// draw a ctf client block
			...
			sprintf(block, "%3d %3d %-12.12s", score, ping,
				ci->name);
...
void SCR_UpdateScreen(void)
{
	...
	for (i = 0; i < numframes; i++) {
		...
		if (scr_draw_loading == 2) {	//  loading plaque over black screen
			...
			scr_draw_loading = false;
			re.DrawGetPicSize(&w, &h, "loading");
			re.DrawPic((viddef.width - w) / 2,
				   (viddef.height - h) / 2, "loading");
			//                        re.EndFrame();
			//                        return;
		}
		...
		// and console specially
			...
			if (cl_drawclock->value) {
				...
				strftime(stime, sizeof(stime),
					 cl_timeformat->string, localtime(&t));
