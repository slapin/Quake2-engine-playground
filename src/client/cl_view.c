...
void V_TestParticles(void)
{
	...
	for (i = 0; i < r_numparticles; i++) {
		...
		for (j = 0; j < 3; j++)
			p->origin[j] =
			    cl.refdef.vieworg[j] + cl.v_forward[j] * d +
...
void V_TestEntities(void)
{
	...
	for (i = 0; i < r_numentities; i++) {
		...
		for (j = 0; j < 3; j++)
			ent->origin[j] =
			    cl.refdef.vieworg[j] + cl.v_forward[j] * f +
			    cl.v_right[j] * r;
...
void V_RenderView(float stereo_separation)
{
	...
	if (cl.frame.valid && (cl.force_refdef || !cl_paused->value)) {
		...
		cl.refdef.fov_y =
		    CalcFov(cl.refdef.fov_x, cl.refdef.width, cl.refdef.height);
