...
void CL_ParseProjectiles(void)
{
	...
	for (i = 0; i < c; i++) {
		...
		if (bits[4] & 128) {
			...
			bits[4] = MSG_ReadByte(&net_message);
			pr.oldorigin[0] =
			    ((bits[0] + ((bits[1] & 15) << 8)) << 1) - 4096;
			pr.oldorigin[1] =
			    (((bits[1] >> 4) + (bits[2] << 4)) << 1) - 4096;
			pr.oldorigin[2] =
			    ((bits[3] + ((bits[4] & 15) << 8)) << 1) - 4096;
		}
...
void CL_AddProjectiles(void)
{
	...
	for (i = 0, pr = cl_projectiles; i < MAX_PROJECTILES; i++, pr++) {
		...
		// interpolate origin
		for (j = 0; j < 3; j++) {
			ent.origin[j] = ent.oldorigin[j] =
			    pr->oldorigin[j] + cl.lerpfrac * (pr->origin[j] -
							      pr->oldorigin[j]);
...
void CL_ParsePacketEntities(frame_t * oldframe, frame_t * newframe)
{
	...
	while (1) {
		...
		if (newnum >= MAX_EDICTS)
			Com_Error(ERR_DROP,
				  "CL_ParsePacketEntities: bad number:%i",
				  newnum);
		...
		if (oldnum > newnum) {	// delta from baseline
			...
			CL_DeltaEntity(newframe, newnum,
				       &cl_entities[newnum].baseline, bits);
			continue;
		}
...
//PGM
				...
				{
					CL_BlasterTrail2(cent->lerp_origin,
							 ent.origin);
...
//PGM
				...
				CL_DiminishingTrail(cent->lerp_origin,
						    ent.origin, cent, effects);
			...
			// RAFAEL
				...
				CL_FlagTrail(cent->lerp_origin, ent.origin,
					     242);
...
//ROGUE
				...
				if (effects & EF_TRACKER) {
					...
					V_AddLight(ent.origin, 155, -1.0, -1.0,
						   -1.0);
				}
...
//======
			...
			// RAFAEL
				...
				CL_IonripperTrail(cent->lerp_origin,
						  ent.origin);
	...
	if (cl.frame.playerstate.pmove.pm_type < PM_DEAD) {	// use predicted values
		...
		for (i = 0; i < 3; i++)
			cl.refdef.viewangles[i] =
			    LerpAngle(ops->viewangles[i], ps->viewangles[i],
				      lerp);
	}
