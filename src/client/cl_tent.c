...
#ifdef QMAX
void CL_Explosion_Particle(vec3_t org, float scale, qboolean large,
			   qboolean rocket);
...
//ROGUE
	cl_mod_explo4_big =
	    re.RegisterModel("models/objects/r_explode2/tris.md2");
	...
	cl_mod_monster_heatbeam =
	    re.RegisterModel("models/proj/widowbeam/tris.md2");
//ROGUE
}
...
void CL_ParseTEnt(void)
{
	...
	case TE_BULLET_SPARKS:
		...
		if (type != TE_SPARKS) {
			...
			cnt = rand() & 15;
			if (cnt == 1)
				S_StartSound(pos, 0, 0, cl_sfx_ric1, 1,
					     ATTN_NORM, 0);
			else if (cnt == 2)
				S_StartSound(pos, 0, 0, cl_sfx_ric2, 1,
					     ATTN_NORM, 0);
			else if (cnt == 3)
				S_StartSound(pos, 0, 0, cl_sfx_ric3, 1,
					     ATTN_NORM, 0);
		}
#ifdef QMAX
		//adding sparks
		{
			...
			CL_ParticleEffectSparks(pos, dir, color,
						(type == TE_GUNSHOT) ? 5 : 10);
		}
#endif
		break;
	...
	case TE_SPLASH:	// bullet hitting water
		...
		if (r == SPLASH_SPARKS) {
			r = rand() & 3;
			if (r == 0)
				S_StartSound(pos, 0, 0, cl_sfx_spark5, 1,
					     ATTN_STATIC, 0);
			else if (r == 1)
				S_StartSound(pos, 0, 0, cl_sfx_spark6, 1,
					     ATTN_STATIC, 0);
			else
				S_StartSound(pos, 0, 0, cl_sfx_spark7, 1,
					     ATTN_STATIC, 0);
		}
		break;
...
#endif
		if (type == TE_GRENADE_EXPLOSION_WATER)
			S_StartSound(pos, 0, 0, cl_sfx_watrexp, 1, ATTN_NORM,
				     0);
		else
			S_StartSound(pos, 0, 0, cl_sfx_grenexp, 1, ATTN_NORM,
				     0);
		break;
...
//        CL_Radius_Explode_SP(pos, 0.6666666);
	...
	case TE_ROCKET_EXPLOSION_WATER:
		...
		if (type == TE_ROCKET_EXPLOSION_WATER)
			...
			S_StartSound(pos, 0, 0, cl_sfx_rockexp, 1, ATTN_NORM,
				     0);
		break;
	...
	case TE_STAIN:		//hahaha server declared stains biotch - psychospaz
		{
			...
			re.AddStain(pos, intens, color[0], color[1], color[2]);
		}
		break;
	case TE_FOOTPRINT:
		{
...
//PMM
		if (cl_mod_heatbeam && (b->model == cl_mod_heatbeam)
			...
			VectorMA(dist, (hand_multiplier * b->offset[0]), r,
				 dist);
