...
void CL_CheckForResend(void)
{
	...
	if (cls.state == ca_disconnected && Com_ServerState()) {
		...
		strncpy(cls.servername, "localhost",
		...
		if (allow_download_players->value) {
			while (precache_check <
				...
				if ((p =
					...
					p = cl.configstrings[CS_PLAYERSKINS +
							     i];
				strcpy(model, p);
				p = strchr(model, '/');
				if (!p)
