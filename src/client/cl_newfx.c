...
//      dec = 0.75;
	...
	while (len > 0) {
		...
		setupParticle(0, 0, 0,
			      ...
			      50, 50, 255,
			      0, 0, 0,
			      1, -0.75, 7.5, 0, particle_generic, 0, NULL, 0);
#else
		if (!free_particles)
			return;
		p = free_particles;
	...
	while (len > 0) {
		...
		if (frand() > 0.3) {
			...
			setupParticle(0, 0, 0,
				      ...
				      0, 0, 0, 1, -1.0 / (3.0 + frand() * 0.5),
				      5, 0, particle_generic, 0, NULL, 0);
#else
			p = free_particles;
			free_particles = p->next;
			p->next = active_particles;
...
//                      p->color = 240;
		...
		setupParticle(0, 0, 0,
			      ...
			      move[2] + dir[2] * 3, 0, 0, 0, 0, 0, 0,
			      200 + rand() * 50, 200 + rand() * 25, rand() * 50,
			      0, 0, 0, 0.5, -1000.0, 3, 1, particle_blaster, 0,
			      NULL, 0);
#else
		for (j = 0; j < 3; j++) {
...
//      AngleVectors (angle_dir, f, r, u);
	...
	for (i = 0; i < self->count; i++) {
		...
		for (j = 0; j < 3; j++) {
			p->org[j] =
			    self->org[j] + self->magnitude * 0.1 * crand();
//                      p->vel[j] = dir[j]*magnitude;
		}
...
void CL_ColorExplosionParticles(vec3_t org, int color8, int run)
{
	...
	for (i = 0; i < 128; i++) {
		...
		setupParticle(0, 0, 0,
			      ...
			      org[1] + ((rand() % 32) - 16),
			      org[2] + ((rand() % 32) - 16),
			      (rand() % 256) - 128, (rand() % 256) - 128,
			      (rand() % 256) - 128, 0, 0, 20,
			      color[0] + (rand() % run),
			      color[1] + (rand() % run),
			      color[2] + (rand() % run), 0, 0, 0, 1.0,
			      -0.4 / (0.6 + frand() * 0.2), 2, 1,
