...
=================
*/
void S_SpatializeOrigin(vec3_t origin, float master_vol, float dist_mult,
		  ...
		  float fvol, float attenuation, float timeofs)
{
	...
	if (sfx->name[0] == '*')
		sfx =
		    S_RegisterSexedSound(&cl_entities[entnum].current,
					 sfx->name);
...
void S_AddLoopSounds(void)
{
	...
	for (i = 0; i < cl.frame.num_entities; i++) {
		...
		for (j = i + 1; j < cl.frame.num_entities; j++) {
			...
			num =
			    (cl.frame.parse_entities +
			     j) & (MAX_PARSE_ENTITIES - 1);
			ent = &cl_parse_entities[num];

			S_SpatializeOrigin(ent->origin, 255.0,
					   SOUND_LOOPATTENUATE, &left, &right);
			left_total += left;
			right_total += right;
		}
