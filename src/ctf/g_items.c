...
qboolean Pickup_Armor(edict_t * ent, edict_t * other)
{
	...
	// use the better armor
		...
		if (newinfo->normal_protection > oldinfo->normal_protection) {
			...
			if (other->client->pers.inventory[old_armor_index] >=
			    newcount)
				return false;
...
void Use_PowerArmor(edict_t * ent, gitem_t * item)
{
	...
	if (ent->flags & FL_POWER_ARMOR) {
		...
		gi.sound(ent, CHAN_AUTO, gi.soundindex("misc/power2.wav"), 1,
			 ATTN_NORM, 0);
	} else {
		...
		ent->flags |= FL_POWER_ARMOR;
		gi.sound(ent, CHAN_AUTO, gi.soundindex("misc/power1.wav"), 1,
			 ATTN_NORM, 0);
	}
}
		...
		csurface_t * surf)
{
	...
	if (taken) {
		...
		if (ent->item->pickup == Pickup_Health) {
			if (ent->count == 2)
				gi.sound(other, CHAN_ITEM,
					 gi.soundindex("items/s_health.wav"), 1,
					 ATTN_NORM, 0);
			else if (ent->count == 10)
				gi.sound(other, CHAN_ITEM,
					 gi.soundindex("items/n_health.wav"), 1,
					 ATTN_NORM, 0);
			else if (ent->count == 25)
				gi.sound(other, CHAN_ITEM,
					 gi.soundindex("items/l_health.wav"), 1,
					 ATTN_NORM, 0);
			else	// (ent->count == 100)
				gi.sound(other, CHAN_ITEM,
					 gi.soundindex("items/m_health.wav"), 1,
					 ATTN_NORM, 0);
		} else if (ent->item->pickup_sound) {
			gi.sound(other, CHAN_ITEM,
				 gi.soundindex(ent->item->pickup_sound), 1,
				 ATTN_NORM, 0);
		}
	}
