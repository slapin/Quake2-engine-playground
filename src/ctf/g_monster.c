...
void M_WorldEffects(edict_t * ent)
{
	...
	if (ent->waterlevel == 0) {
		if (ent->flags & FL_INWATER) {
			gi.sound(ent, CHAN_BODY,
				 gi.soundindex("player/watr_out.wav"), 1,
				 ATTN_NORM, 0);
	...
	if ((ent->watertype & CONTENTS_LAVA) && !(ent->flags & FL_IMMUNE_LAVA)) {
		if (ent->damage_debounce_time < level.time) {
			...
			T_Damage(ent, world, world, vec3_origin, ent->s.origin,
				 vec3_origin, 10 * ent->waterlevel, 0, 0,
				 MOD_LAVA);
		}
	}
	if ((ent->watertype & CONTENTS_SLIME)
		...
		if (ent->damage_debounce_time < level.time) {
			...
			T_Damage(ent, world, world, vec3_origin, ent->s.origin,
				 vec3_origin, 4 * ent->waterlevel, 0, 0,
				 MOD_SLIME);
		}
	}
...
void M_SetEffects(edict_t * ent)
{
	...
	if (ent->powerarmor_time > level.time) {
		...
		} else if (ent->monsterinfo.power_armor_type ==
			   POWER_ARMOR_SHIELD) {
			ent->s.effects |= EF_COLOR_SHELL;
			...
			if (strcmp(target->classname, "point_combat") != 0) {
				gi.dprintf
				    ("%s at (%i %i %i) has a bad combattarget %s : %s at (%i %i %i)\n",
				     ...
				     (int)self->s.origin[1],
				     (int)self->s.origin[2], self->combattarget,
				     target->classname,
