		...
		if (tr.contents & MASK_WATER) {
			...
			if (!VectorCompare(start, tr.endpos)) {
				if (tr.contents & CONTENTS_WATER) {
					if (strcmp(tr.surface->name, "*brwater")
					    == 0)
			...
			tr = gi.trace(water_start, NULL, NULL, end, self,
				      MASK_SHOT);
		}
	}
	// send gun puff / flash
			  ...
			  csurface_t * surf)
{
	...
	if (!other->takedamage) {
		if (ent->spawnflags & 1) {
			...
			gi.sound(ent, CHAN_VOICE,
				 gi.soundindex("weapons/grenlb1b.wav"), 1,
				 ATTN_NORM, 0);
		}
		return;
	}
		   ...
		   int speed, float timer, float damage_radius, qboolean held)
{
	...
	if (timer <= 0.0)
		...
		gi.sound(self, CHAN_WEAPON,
			 gi.soundindex("weapons/hgrent1a.wav"), 1, ATTN_NORM,
			 0);
		gi.linkentity(grenade);
	}
}
...
void bfg_explode(edict_t * self)
{
	...
	if (self->s.frame == 0) {
		...
		while ((ent =
			...
			gi.multicast(ent->s.origin, MULTICAST_PHS);
			T_Damage(ent, self, self->owner, self->velocity,
				 ent->s.origin, vec3_origin, (int)points, 0,
				 DAMAGE_ENERGY, MOD_BFG_EFFECT);
		}
	}
...
void bfg_touch(edict_t * self, edict_t * other, cplane_t * plane,
	       csurface_t * surf)
{
	if (other == self->owner)
		return;
	...
	self->touch = NULL;
	VectorMA(self->s.origin, -1 * FRAMETIME, self->velocity,
		 self->s.origin);
...
//ZOID
		...
		if (ctf->value && ent->client &&
		    ...
		    self->owner->client->resp.ctf_team)
			continue;
//ZOID
		...
		while (1) {
			...
			if ((tr.ent->takedamage)
			    ...
			    && (tr.ent != self->owner))
				T_Damage(tr.ent, self, self->owner, dir,
					 tr.endpos, vec3_origin, dmg, 1,
