...
void trigger_key_use(edict_t * self, edict_t * other, edict_t * activator)
{
	...
	if (!activator->client->pers.inventory[index]) {
		...
		gi.sound(activator, CHAN_AUTO, gi.soundindex("misc/keytry.wav"),
			 1, ATTN_NORM, 0);
		return;
	}
...
void trigger_counter_use(edict_t * self, edict_t * other, edict_t * activator)
{
	...
	if (self->count) {
		if (!(self->spawnflags & 1)) {
			gi.centerprintf(activator, "%i more to go...",
					self->count);
			gi.sound(activator, CHAN_AUTO,
				 gi.soundindex("misc/talk1.wav"), 1, ATTN_NORM,
				 0);
		}
		return;
	}
	...
	if (strcmp(other->classname, "grenade") == 0) {
		...
		if (other->client) {
			...
			if (other->fly_sound_debounce_time < level.time) {
				other->fly_sound_debounce_time =
				    level.time + 1.5;
				gi.sound(other, CHAN_AUTO, windsound, 1,
					 ATTN_NORM, 0);
			}
		}
	}
		...
		csurface_t * surf)
{
	...
	if (!(self->spawnflags & 4)) {
		if ((level.framenum % 10) == 0)
			gi.sound(other, CHAN_AUTO, self->noise_index, 1,
				 ATTN_NORM, 0);
	}
