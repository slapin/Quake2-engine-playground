...
void P_DamageFeedback(edict_t * player)
{
	...
	if ((level.time > player->pain_debounce_time)
		...
		gi.sound(player, CHAN_VOICE,
			 gi.soundindex(va("*pain%i_%i.wav", l, r)), 1,
			 ATTN_NORM, 0);
	}
	...
	if (client->damage_parmor)
		VectorMA(v, (float)client->damage_parmor / realcount,
			 power_color, v);
...
void SV_CalcGunOffset(edict_t * ent)
{
	...
	if (bobcycle & 1) {
		ent->client->ps.gunangles[ROLL] =
		    -ent->client->ps.gunangles[ROLL];
		ent->client->ps.gunangles[YAW] =
		    -ent->client->ps.gunangles[YAW];
	}
	...
	if (ent->client->quad_framenum > level.framenum) {
		...
		if (remaining == 30)	// beginning to fade
			gi.sound(ent, CHAN_ITEM,
				 gi.soundindex("items/damage2.wav"), 1,
				 ATTN_NORM, 0);
		...
		if (remaining == 30)	// beginning to fade
			gi.sound(ent, CHAN_ITEM,
				 gi.soundindex("items/protect2.wav"), 1,
				 ATTN_NORM, 0);
		if (remaining > 30 || (remaining & 4))
		...
		if (remaining == 30)	// beginning to fade
			gi.sound(ent, CHAN_ITEM,
				 gi.soundindex("items/airout.wav"), 1,
				 ATTN_NORM, 0);
		if (remaining > 30 || (remaining & 4))
...
void P_WorldEffects(void)
{
	...
	if (!old_waterlevel && waterlevel) {
		...
		if (current_player->watertype & CONTENTS_LAVA)
			gi.sound(current_player, CHAN_BODY,
				 gi.soundindex("player/lava_in.wav"), 1,
				 ATTN_NORM, 0);
	...
	if (waterlevel == 3) {
		...
		if (breather || envirosuit) {
			...
			if (((int)
			     (current_client->breather_framenum -
			      level.framenum) % 25) == 0) {
		...
		if (current_player->air_finished < level.time) {	// drown!
			if (current_player->client->next_drown_time < level.time
				...
				current_player->client->next_drown_time =
				    level.time + 1;
				...
				if (current_player->health <=
					...
					gi.sound(current_player, CHAN_VOICE,
						 gi.soundindex("*gurp1.wav"), 1,
						 ATTN_NORM, 0);
				else
					gi.sound(current_player, CHAN_VOICE,
						 gi.soundindex("*gurp2.wav"), 1,
						 ATTN_NORM, 0);
				...
				T_Damage(current_player, world, world,
					 ...
					 vec3_origin, current_player->dmg, 0,
	...
	if (waterlevel
		...
		if (current_player->watertype & CONTENTS_LAVA) {
			...
			if (envirosuit)	// take 1/3 damage with envirosuit
				T_Damage(current_player, world, world,
					 vec3_origin, current_player->s.origin,
					 vec3_origin, 1 * waterlevel, 0, 0,
					 MOD_LAVA);
			else
				T_Damage(current_player, world, world,
					 vec3_origin, current_player->s.origin,
					 vec3_origin, 3 * waterlevel, 0, 0,
					 MOD_LAVA);
		}
...
//update chasecam follower stats
	for (i = 1; i <= maxclients->value; i++) {
		...
		memcpy(e->client->ps.stats,
		       ent->client->ps.stats, sizeof(ent->client->ps.stats));
