	     ...
	     int *height)
{
	...
	*pic = out;
	...
	if (raw - (byte *) pcx > len) {
		ri.Con_Printf(PRINT_DEVELOPER, "PCX file %s was malformed",
			      filename);
		free(*pic);
		*pic = NULL;
	}
...
void LoadTGA(char *name, byte ** pic, int *width, int *height)
{
	...
	*pic = NULL;
	...
	if (targa_header.image_type != 2 && targa_header.image_type != 10) {
		ri.Con_Printf(PRINT_ALL,
			      "LoadTGA: %s has wrong image format; only type 2 and 10 targa RGB images supported.\n",
			      name);
		ri.FS_FreeFile(buffer);
		return;
	}
	if (targa_header.colormap_type != 0
		...
		ri.Con_Printf(PRINT_ALL,
			      "LoadTGA: %s has wrong image format; only 32 or 24 bit images supported (no colormaps).\n",
			      name);
		ri.FS_FreeFile(buffer);
		return;
	}
...
// Knightmare- check for bad data 
	if (rawdata[6] != 'J'
	    || rawdata[7] != 'F' || rawdata[8] != 'I' || rawdata[9] != 'F') {
	...
	*height = cinfo.output_height;
	...
	if (!scanline) {
		ri.Con_Printf(PRINT_ALL,
			      "Insufficient RAM for JPEG scanline buffer\n");
		free(rgbadata);
		jpeg_destroy_decompress(&cinfo);
		ri.FS_FreeFile(rawdata);
		return;
			...
			int outwidth, int outheight)
{
	...
	for (i = 0; i < outheight; i++, out += outwidth) {
		...
		inrow2 =
		    in + inwidth * (int)((i + 0.75) * inheight / outheight);
		frac = fracstep >> 1;
...
void GL_MipMap(byte * in, int width, int height)
{
	...
	for (i = 0; i < height; i++, in += width) {
		for (j = 0; j < width; j += 8, out += 4, in += 8) {
			out[0] =
			    (in[0] + in[4] + in[width + 0] +
			     in[width + 4]) >> 2;
			out[1] =
			    (in[1] + in[5] + in[width + 1] +
			     in[width + 5]) >> 2;
			out[2] =
			    (in[2] + in[6] + in[width + 2] +
			     in[width + 6]) >> 2;
			out[3] =
			    (in[3] + in[7] + in[width + 3] +
			     in[width + 7]) >> 2;
		}
	}
}
...
#ifdef USE_GLU
	if (mipmap)
		gluBuild2DMipmaps(GL_TEXTURE_2D, samples, width, height,
				  GL_RGBA, GL_UNSIGNED_BYTE, trans);
	else if (scaled_width == width && scaled_height == height)
		qglTexImage2D(GL_TEXTURE_2D, 0, comp, width, height, 0, GL_RGBA,
			      GL_UNSIGNED_BYTE, trans);
	else {
		gluScaleImage(GL_RGBA, width, height, GL_UNSIGNED_BYTE, trans,
			      scaled_width, scaled_height, GL_UNSIGNED_BYTE,
			      scaled);
	...
	if (mipmap) {
		...
		while (scaled_width > 1 || scaled_height > 1) {
			...
			if (qglColorTableEXT && gl_ext_palettedtexture->value
				...
				qglTexImage2D(GL_TEXTURE_2D, miplevel, comp,
					      scaled_width, scaled_height, 0,
	...
	else*/
	{
		for (i = 0; i < s; i++) {
			...
			if (p == 255) {	// transparent, so scan around for another color
				...
				// copy rgb components
				((byte *) & trans[i])[0] =
				    ((byte *) & d_8to24table[p])[0];
				((byte *) & trans[i])[1] =
				    ((byte *) & d_8to24table[p])[1];
				((byte *) & trans[i])[2] =
				    ((byte *) & d_8to24table[p])[2];
			}
		}
		    ...
		    imagetype_t type, int bits)
{
	...
	if (strlen(name) >= sizeof(image->name))
		ri.Sys_Error(ERR_DROP, "Draw_LoadPic: \"%s\" is too long",
			     name);
	...
	if (!strcmp(s + len - 4, ".tga") || !strcmp(s + len - 4, ".jpg")
	    || !strcmp(s + len - 4, ".png")) {
		s[len - 3] = 'w';
		s[len - 2] = 'a';
		s[len - 1] = 'l';
	...
	if ((image->type == it_pic) && bits == 8
		...
		for (i = 0; i < image->height; i++)
			for (j = 0; j < image->width; j++, k++)
				scrap_texels[texnum][(y + i) * BLOCK_WIDTH + x +
						     j] = pic[k];
...
image_t *GL_FindImage(char *name, imagetype_t type)
{
	...
	if (!strcmp(name + len - 4, ".pcx")) {
		...
		for (i = 0; i < IMAGETYPES && !image; i++)
			image =
			    GL_FindImage(va("%s%s", basename, image_types[i]),
					 type);
...
void GL_FreeUnusedImages(void)
{
	...
	for (i = 0; i < PARTICLE_TYPES; i++)
		if (r_particletextures[i])	//dont mess with null ones silly :p
			r_particletextures[i]->registration_sequence =
