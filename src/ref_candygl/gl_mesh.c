		  ...
		  float frontv[3], float backv[3])
{
	...
	if (currententity->
		...
		for (i = 0; i < nverts; i++, v++, ov++, lerp += 4) {
			...
			lerp[0] =
			    move[0] + ov->v[0] * backv[0] +
			    v->v[0] * frontv[0] + normal[0] * POWERSUIT_SCALE;
			lerp[1] =
			    move[1] + ov->v[1] * backv[1] +
			    v->v[1] * frontv[1] + normal[1] * POWERSUIT_SCALE;
			lerp[2] =
			    move[2] + ov->v[2] * backv[2] +
			    v->v[2] * frontv[2] + normal[2] * POWERSUIT_SCALE;
		}
	} else {
		for (i = 0; i < nverts; i++, v++, ov++, lerp += 4) {
			lerp[0] =
...
void GL_DrawOutLine(dmdl_t * paliashdr, int posenum, float width)
{
	...
	{
		vec3_t length;
		VectorSubtract(r_newrefdef.vieworg, currententity->origin,
			       length);
...
//      glScalef (frame->scale[0], frame->scale[1], frame->scale[2]);
	...
	GL_LerpVerts(paliashdr->num_xyz, v, ov, verts, lerp, move, frontv,
		     backv);
	...
	while (1) {
		...
		do {
			if (true)	//relative origin/angle -- faster...
			{
				...
				memcpy(point, s_lerped[order[2]],
				       sizeof(point));

				point[0] -=
				    shadevector[0] * (point[2] + lheight);
				point[1] -=
				    shadevector[1] * (point[2] + lheight);
				point[2] = height;
			} else	//absolute origin/angle -- much slower...
			{
				memcpy(temp[0], s_lerped[order[2]],
				       sizeof(temp[0]));

				RotatePointAroundVector(temp[1], forward,
							temp[0], angles[ROLL]);
				RotatePointAroundVector(temp[2], up, temp[0],
							angles[YAW]);
				RotatePointAroundVector(temp[3], right, temp[2],
							angles[PITCH]);

				VectorAdd(temp[3], currententity->origin,
					  point);
...
static qboolean R_CullAliasModel(vec3_t bbox[8], entity_t * e)
{
	...
	if ((e->frame >= paliashdr->num_frames) || (e->frame < 0)) {
		ri.Con_Printf(PRINT_ALL,
			      "R_CullAliasModel %s: no such frame %d\n",
			      currentmodel->name, e->frame);
		e->frame = 0;
	}
	if ((e->oldframe >= paliashdr->num_frames) || (e->oldframe < 0)) {
		ri.Con_Printf(PRINT_ALL,
			      "R_CullAliasModel %s: no such oldframe %d\n",
...

	{
		...
		for (p = 0; p < 8; p++) {
			...
			for (f = 0; f < 4; f++) {
				float dp =
				    DotProduct(frustum[f].normal, bbox[p]);
...
void R_DrawAliasModel(entity_t * e)
{
	...
	vec3_t bbox[8];
	image_t *skin;
	qboolean mirrormodel = currententity->flags & RF_MIRRORMODEL
	    || (r_lefthand->value == 1.0F
		&& currententity->flags & RF_WEAPONMODEL);

	if (!
	    (e->flags & RF_WEAPONMODEL || e->flags & RF_VIEWERMODEL
	     || e->renderfx & RF2_CAMERAMODEL)) {
...
// =================
	...
	if ((currententity->frame >= paliashdr->num_frames)
		...
		ri.Con_Printf(PRINT_ALL,
			      "R_DrawAliasModel %s: no such frame %d\n",
	...
	if ((currententity->oldframe >= paliashdr->num_frames)
	    || (currententity->oldframe < 0)) {
		ri.Con_Printf(PRINT_ALL,
			      "R_DrawAliasModel %s: no such oldframe %d\n",
	...
	if (!(r_newrefdef.rdflags & RDF_NOWORLDMODEL))	// no shadows
		if (!(currententity->renderfx & RF2_NOSHADOW) &&
			...
			case 2:
				...
				R_ShadowLight(currententity->origin,
					      shadevector);
			default:
				{
					if (currententity->
