...
void R_RenderDlight(dlight_t * light)
{
	...
	if (VectorLength(v) < rad) {	// view is inside the dlight
		V_AddBlend(light->color[0], light->color[1], light->color[2],
			   light->intensity * 0.0003, v_blend);
		return;
	}
#endif
	...
	for (i = 0; i < node->numsurfaces; i++, surf++) {
		...
		if (lightmap) {
			...
			for (maps = 0;
				...
				pointcolor[0] +=
				    lightmap[0] * scale[0] * (DIV255);
				pointcolor[1] +=
				    lightmap[1] * scale[1] * (DIV255);
				pointcolor[2] +=
				    lightmap[2] * scale[2] * (DIV255);
				lightmap +=
				    3 * ((surf->extents[0] >> 4) +
					 1) * ((surf->extents[1] >> 4) + 1);
			}
		}
...
void R_StainNode(dstain_t * st, mnode_t * node)
{
	...
	for (c = node->numsurfaces, surf =
		...
		if ((tex->flags & (SURF_SKY | SURF_TRANS33 | SURF_TRANS66 |
				   SURF_WARP)))
			continue;

		frad = st->intensity;
		fdist =
		    DotProduct(st->origin,
		...
		for (i = 0; i < 3; i++)
			impact[i] =
			    st->origin[i] - surf->plane->normal[i] * fdist;
	...
	for (lnum = 0; lnum < r_newrefdef.num_dlights; lnum++) {
		...
		for (t = 0, ftacc = 0; t < tmax; t++, ftacc += 16) {
			...
			for (s = 0, fsacc = 0; s < smax;
				...
				if (fdist < fminlight) {
					pfBL[0] +=
					    (frad - fdist) * dl->color[0];
					pfBL[1] +=
					    (frad - fdist) * dl->color[1];
					pfBL[2] +=
					    (frad - fdist) * dl->color[2];
				}
			}
		}
	}
