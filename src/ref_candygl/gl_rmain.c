...
sortedent_t NewSortEnt(entity_t * ent, qboolean waterstart)
{
	...
	s_ent.inwater =
	    (Mod_PointInLeaf(ent->origin, r_worldmodel)->contents & MASK_WATER);
...
void R_SortEntitiesOnList(qboolean inwater)
{
	...
	qsort((void *)theents, r_newrefdef.num_entities, sizeof(sortedent_t),
	      transCompare);
}
...
sortedpart_t NewSortPart(particle_t * p)
{
	...
	s_part.len =
	    ...
	    (result[2] * result[2]);
	s_part.inwater =
	    (Mod_PointInLeaf(p->origin, r_worldmodel)->contents & MASK_WATER);
...
void R_SortParticlesOnList(int num_particles, const particle_t particles[])
{
	...
	qsort((void *)theparts, num_particles, sizeof(sortedpart_t),
	      transCompare);
}
...
void GL_DrawParticles(int num_particles, qboolean inWater)
{
	...
	vec3_t right =
	    { vright[0] * 0.75f, vright[1] * 0.75f, vright[2] * 0.75f };
	...
	for (i = 0; i < num_particles; i++) {
		if (!(r_newrefdef.rdflags & RDF_NOWORLDMODEL)) {
			if (gl_transrendersort->value == 2)	//sorted with ents etc
			{
				...
				if ((inWater && !theparts[i].inwater)
				    || (!inWater && theparts[i].inwater))
					continue;

				if (gl_particledistance->value > 0
				    && theparts[i].len >
				    (100.0 * gl_particledistance->value))
					continue;
		...
		if (p->flags & PART_SHADED) {
			...
			shadelight[0] =
			    (lighting * shadelight[0] +
			     (1 - lighting)) * p->red;
			shadelight[1] =
			    (lighting * shadelight[1] +
			     (1 - lighting)) * p->green;
			shadelight[2] =
			    (lighting * shadelight[2] +
			     (1 - lighting)) * p->blue;
			...
			qglColor4ub(shadelight[0], shadelight[1], shadelight[2],
				    alpha);
		} else
...
#define DEPTHHACK_RANGE_LONG	0.95f
		...
		if (p->flags & PART_DEPTHHACK_SHORT)	//nice little poly-peeking - psychospaz
			qglDepthRange(gldepthmin,
				      gldepthmin +
				      DEPTHHACK_RANGE_SHORT * (gldepthmax -
							       gldepthmin));
		if (p->flags & PART_DEPTHHACK_MID)
			qglDepthRange(gldepthmin,
				      gldepthmin +
				      DEPTHHACK_RANGE_MID * (gldepthmax -
							     gldepthmin));
		if (p->flags & PART_DEPTHHACK_LONG)
			qglDepthRange(gldepthmin,
				      gldepthmin +
				      DEPTHHACK_RANGE_LONG * (gldepthmax -
							      gldepthmin));
...
#define MAGICNUMBER 32
		...
		{		//p->angle = start
			//p->origin= end
			float tlen, halflen, len, dec =
			    size, warpfactor, warpadd, oldwarpadd, warpsize,
			    time, i = 0, factor;
...
#define LIGHTNINGWARPFUNCTION 0.25*sin(time+i+warpfactor)*(dec/len)
			...
			while (len > dec) {
				...
				VectorSubtract(lastvec, ang_right,
					       angl_coord[2]);
				VectorSubtract(thisvec, ang_right,
					       angl_coord[3]);
				...
				qglPushMatrix();
				{
					qglBegin(GL_QUADS);
					{
						...
						qglVertex3fv(angl_coord[0]);
						qglTexCoord2f(0 + oldwarpadd,
							      0);
						qglVertex3fv(angl_coord[1]);
						qglTexCoord2f(1 + oldwarpadd,
							      0);
						qglVertex3fv(angl_coord[2]);
				...
				tlen =
				    (len <
				     halflen) ? fabs(len - halflen) : fabs(len -
									   halflen);
				...
				thisvec[0] =
				    ((thisvec[0] + (size)) +
				     tempvec[0] * factor) / (factor + 1);
				thisvec[1] =
				    ((thisvec[1] + (size)) +
				     tempvec[1] * factor) / (factor + 1);
				thisvec[2] =
				    ((thisvec[2] + (size)) +
			...
			if (len > 0) {
				...
				VectorSubtract(lastvec, ang_right,
					       angl_coord[2]);
				VectorSubtract(p->origin, ang_right,
					       angl_coord[3]);
			...
			//never dissapears because of angle like other engines :D
		{
			...
			if (false) {
				...
				AngleVectors(r_newrefdef.viewangles, tempfor,
					     NULL, NULL);
				...
				VectorSubtract(p->origin, r_newrefdef.vieworg,
					       vdelta);
				Com_Printf("up:%f right:%f\n",
					   DotProduct(vdelta, ang_up),
					   DotProduct(vdelta, ang_right));

				ProjectPointOnPlane(temp_up[2], ang_up,
						    tempfor);
			...
			qglPushMatrix();
			{
				qglTranslatef(p->origin[0], p->origin[1],
					      p->origin[2]);
				...
				qglBegin(GL_QUADS);
				{
					if (p->angle[2])	//if we have roll, we do calcs
					{
						...
						vectoanglerolled(angl_coord[0],
								 p->angle[2],
								 angl_coord[1]);
						AngleVectors(angl_coord[1],
							     ang_forward,
							     ang_right, ang_up);

						VectorScale(ang_forward, 0.75f,
							    ang_forward);
						VectorScale(ang_right, 0.75f,
							    ang_right);
						VectorScale(ang_up, 0.75f,
							    ang_up);

						VectorAdd(ang_up, ang_right,
							  angl_coord[0]);
						VectorSubtract(ang_right,
							       ang_up,
							       angl_coord[1]);
						VectorNegate(angl_coord[0],
							     angl_coord[2]);
						VectorNegate(angl_coord[1],
							     angl_coord[3]);
...
//      float   yfov;
	...
	y2 = ceil(vid.height -
		  (r_newrefdef.y +
		   r_newrefdef.height) * vid.height / vid.height);
...
void R_RenderView(refdef_t * fd)
{
	...
	if (!(r_newrefdef.rdflags & RDF_NOWORLDMODEL)
	    && gl_transrendersort->value) {
		R_SortParticlesOnList(r_newrefdef.num_particles,
...
void R_Register(void)
{
	...
	gl_transrendersort =
	    ri.Cvar_Get("gl_transrendersort", "1", CVAR_ARCHIVE);
	gl_particlelighting =
	    ri.Cvar_Get("gl_particlelighting", "0.75", CVAR_ARCHIVE);
	gl_particledistance =
	    ri.Cvar_Get("gl_particledistance", "0", CVAR_ARCHIVE);
...
#endif
	gl_texturemode =
	    ri.Cvar_Get("gl_texturemode", "GL_LINEAR_MIPMAP_NEAREST",
			CVAR_ARCHIVE);
	gl_texturealphamode =
	...
	r_cellshading_width =
	    ri.Cvar_Get("r_cellshading_width", "4", CVAR_ARCHIVE);
...
qboolean R_SetMode(void)
{
	...
	if ((err =
	     GLimp_SetMode(&vid.width, &vid.height, gl_mode->value,
			   fullscreen)) == rserr_ok) {
		...
		if (err == rserr_invalid_fullscreen) {
			...
			vid_fullscreen->modified = false;
			ri.Con_Printf(PRINT_ALL,
				      "rfx_gl::R_SetMode() - fullscreen unavailable in this mode\n");
			if ((err =
			     GLimp_SetMode(&vid.width, &vid.height,
...
int R_Init(void *hinstance, void *hWnd)
{
	...
	if (!QGL_Init(gl_driver->string)) {
		QGL_Shutdown();
		ri.Con_Printf(PRINT_ALL,
			      "rfx_gl::R_Init() - could not load \"%s\"\n",
			      gl_driver->string);
		return -1;
	}
	...
	if (strstr(gl_config.extensions_string, "GL_EXT_compiled_vertex_array")
		...
		qglLockArraysEXT =
		    (void *)qwglGetProcAddress("glLockArraysEXT");
		qglUnlockArraysEXT =
		    (void *)qwglGetProcAddress("glUnlockArraysEXT");
	} else {
		ri.Con_Printf(PRINT_ALL,
...
void R_BeginFrame(float camera_separation)
{
	...
	if (vid_gamma->modified) {
		...
		if (gl_config.renderer & (GL_RENDERER_VOODOO)) {
			...
			Com_sprintf(envbuffer, sizeof(envbuffer),
				    "SSTV2_GAMMA=%f", g);
			putenv(envbuffer);
			Com_sprintf(envbuffer, sizeof(envbuffer),
				    "SST_GAMMA=%f", g);
...
** R_DrawBeam
*/
void R_RenderBeam(vec3_t start, vec3_t end, float size, float red, float green,
