...
void DrawGLFlowingPoly(msurface_t * fa)
{
	...
	scroll =
	    -64 * ((r_newrefdef.time / 40.0) - (int)(r_newrefdef.time / 40.0));
	if (scroll == 0.0)
		scroll = -64.0;
...
void R_BlendLightmaps(void)
{
	...
	if (gl_dynamic->value) {
		...
		for (surf = gl_lms.lightmap_surfaces[0]; surf != 0;
			...
			if (LM_AllocBlock
			    (smax, tmax, &surf->dlight_s, &surf->dlight_t)) {
				base = gl_lms.lightmap_buffer;
				base +=
				    (surf->dlight_t * BLOCK_WIDTH +
