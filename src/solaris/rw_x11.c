...
void st2_fixup(XImage * framebuf, int x, int y, int width, int height)
{
	...
	for (yi = y; yi < (y + height); yi++) {
		src =
		    (unsigned char *)&framebuf->data[yi *
						     framebuf->bytes_per_line];
...
static qboolean SWimp_InitGraphics(qboolean fullscreen)
{
	...
	if (num_visuals == 0) {
		...
		template.screen = XDefaultScreen(dpy);
		template_mask =
		    VisualDepthMask | VisualClassMask | VisualScreenMask;
		x_visinfo =
	...
	if (XShmQueryExtension(dpy)) {
		...
		if (displayname) {
			...
			if (*d)
				*d = 0;
			if (!
			    (!strcasecmp(displayname, "unix") || !*displayname))
				doShm = false;
		}
	}
...
void SWimp_EndFrame(void)
{
	...
	if (doShm) {
		...
		vid.buffer =
		    (unsigned char *)x_framebuffer[current_framebuffer]->data;
		XSync(dpy, False);
	} else {
		if (x_visinfo->depth == 16)
			st2_fixup(x_framebuffer[current_framebuffer], 0, 0,
			...
			st3_fixup(x_framebuffer[current_framebuffer], 0, 0,
				  vid.width, vid.height);
		XPutImage(dpy, win, x_gc, x_framebuffer[0], 0, 0, 0, 0,
			  vid.width, vid.height);
		XSync(dpy, False);
	}
}
...
void SWimp_SetPalette(const unsigned char *palette)
{
	...
	for (i = 0; i < 256; i++) {
		st2d_8to16table[i] =
		    xlib_rgb16(palette[i * 4], palette[i * 4 + 1],
			       palette[i * 4 + 2]);
		st2d_8to24table[i] =
