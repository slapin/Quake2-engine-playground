 ...
 */
int send_newt_video(Display * disp, Window win, void *data, int sw, int sh)
{
...
static void setup_video(Display * disp, Window win)
{
	...
	if ((session != NULL)
	    && ((nvsession = nvSessionSetup(session)) != NULL)) {
		struct sigaction sa;
...
static void nvSessionCheck(nvSession sess)
{
	...
	if ((new_target == NULL) || (target == NULL)
	    || strcmp(new_target, target)) {
		if (newtnv != NULL) {
