...
// set sample bits & speed
	...
	if ((int)sndspeed->value > 0) {
		...
		au_info.play.encoding =
		    (au_info.play.precision == 8
		     ? AUDIO_ENCODING_LINEAR8 : AUDIO_ENCODING_LINEAR);
	...
	DPRINTF
	    ("channels %d, speed %d, log2(samples) %d, submission chunk %d\n",
	     dma.channels, dma.speed, i - 1, dma.submission_chunk);
...
void SNDDMA_Submit(void)
{
	...
	if (last_chunk_idx == -1) {
		if (write(audio_fd, dma.buffer, dma.samples * samplebytes) !=
		    dma.samples * samplebytes)
	...
	if (au_info.play.error) {
		...
		if (write(audio_fd, dma.buffer, dma.samples * samplebytes) !=
		    dma.samples * samplebytes)
			Com_Printf
			    ("refill sound driver after underflow failed\n");
		last_chunk_idx = 0;
		dma.samplepos = 0;
		return;
	}
	...
	while (chunk_idx != last_chunk_idx) {
		...
		if (write(audio_fd,
			  ...
			  dma.submission_chunk * samplebytes) !=
		    dma.submission_chunk * samplebytes) {
