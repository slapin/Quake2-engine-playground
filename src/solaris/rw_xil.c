...
void ResetXILFrameBuffers(void)
{
	...
	if (use_truecolour) {
		...
		tmp8to24_image =
		    xil_create(xil_state, vid.width, vid.height, 3, XIL_BYTE);
		xil_8to24_lookup =
		    xil_lookup_create(xil_state, XIL_BYTE, XIL_BYTE, 3, 256, 0,
				      tmp_palette);
	}
}
...
void GetEvent(void)
{
	...
	case MotionNotify:
		if (_windowed_mouse->value) {
			int xoffset =
			    ((int)x_event.xmotion.x - (int)(window_width / 2));
			int yoffset =
			    ((int)x_event.xmotion.y - (int)(window_height / 2));
			...
			if (config_notify_height > config_notify_width / aspect) {
				...
				XResizeWindow(x_disp, x_win,
					      ...
					      (int)(config_notify_width /
						    aspect));
			} else if (config_notify_width >
				   config_notify_height * aspect) {
				force_resize = 1;
				XResizeWindow(x_disp, x_win,
...
static qboolean SWimp_InitGraphics(qboolean fullscreen)
{
	...
	if (xil_force_truecolour->value || num_visuals == 0) {
		...
		x_visinfo =
		    XGetVisualInfo(x_disp, template_mask, &template,
				   &num_visuals);
		if (num_visuals == 0) {
	...
	// setup attributes for main window
	{
		...
		x_win = XCreateWindow(x_disp, XRootWindow(x_disp, x_visinfo->screen), 0, 0,	// x, y
				      window_width, window_height, 0,	// borderwidth
				      x_visinfo->depth,
	...
	if (x_visinfo->depth == 8) {
		...
		if (x_visinfo->class == PseudoColor) {
			x_cmap =
			    XCreateColormap(x_disp, x_win, x_vis, AllocAll);
