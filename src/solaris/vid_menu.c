...
static void BrightnessCallback(void *s)
{
	...
	if (s_current_menu_index == 0)
		s_brightness_slider[1].curvalue =
		    s_brightness_slider[0].curvalue;
	else
		s_brightness_slider[0].curvalue =
		    s_brightness_slider[1].curvalue;
...
static void ApplyChanges(void *unused)
{
	...
	gamma =
	    (0.8 -
	     (s_brightness_slider[s_current_menu_index].curvalue / 10.0 -
	      0.5)) + 0.5;
	...
	Cvar_SetValue("vid_fullscreen",
		      s_fs_box[s_current_menu_index].curvalue);
	Cvar_SetValue("gl_ext_palettedtexture",
		      s_paletted_texture_box.curvalue);
	...
	if (stricmp(vid_ref->string, "gl") == 0) {
		if (vid_gamma->modified) {
			...
			if (stricmp(gl_driver->string, "3dfxgl") == 0) {
				...
				g = 2.00 * (0.8 - (vid_gamma->value - 0.5)) +
				    1.0F;
...
void VID_MenuInit(void)
{
	...
	for (i = 0; i < 2; i++) {
		...
		s_brightness_slider[i].curvalue =
		    (1.3 - vid_gamma->value + 0.5) * 10;
	...
	Menu_AddItem(&s_software_menu,
		     (void *)&s_screensize_slider[SOFTWARE_MENU]);
	Menu_AddItem(&s_software_menu,
		     (void *)&s_brightness_slider[SOFTWARE_MENU]);
	...
	Menu_AddItem(&s_software_menu,
		     (void *)&s_defaults_action[SOFTWARE_MENU]);
