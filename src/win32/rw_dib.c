...
qboolean DIB_Init(unsigned char **ppbuffer, int *ppitch)
{
	...
	sww_state.hDIBSection = CreateDIBSection(sww_state.hDC,
						 pbmiDIB,
						 DIB_RGB_COLORS,
						 &sww_state.pDIBBase, NULL, 0);

	if (sww_state.hDIBSection == NULL) {
		ri.Con_Printf(PRINT_ALL,
			      "DIB_Init() - CreateDIBSection failed\n");
		goto fail;
	}
	...
	if ((sww_state.hdcDIBSection =
	     CreateCompatibleDC(sww_state.hDC)) == NULL) {
		ri.Con_Printf(PRINT_ALL,
			      "DIB_Init() - CreateCompatibleDC failed\n");
		goto fail;
	}
	if ((previously_selected_GDI_obj =
	     SelectObject(sww_state.hdcDIBSection,
...
void DIB_SetPalette(const unsigned char *_pal)
{
	...
	if (sww_state.hdcDIBSection) {
		...
		if (SetDIBColorTable(sww_state.hdcDIBSection, 0, 256, colors) ==
		    0) {
			ri.Con_Printf(PRINT_ALL,
	...
	if (sww_state.palettized) {
		...
		if (SetSystemPaletteUse(hDC, SYSPAL_NOSTATIC) == SYSPAL_ERROR) {
			ri.Sys_Error(ERR_FATAL,
				     "DIB_SetPalette() - SetSystemPaletteUse() failed\n");
		}
		...
		if ((sww_state.hPal = CreatePalette(pLogPal)) == NULL) {
			ri.Sys_Error(ERR_FATAL,
				     "DIB_SetPalette() - CreatePalette failed(%x)\n",
				     GetLastError());
		}

		if ((hpalOld =
		     SelectPalette(hDC, sww_state.hPal, FALSE)) == NULL) {
			ri.Sys_Error(ERR_FATAL,
				     "DIB_SetPalette() - SelectPalette failed(%x)\n",
				     GetLastError());
		}
		...
		if ((ret = RealizePalette(hDC)) != pLogPal->palNumEntries) {
			ri.Sys_Error(ERR_FATAL,
				     "DIB_SetPalette() - RealizePalette set %d entries\n",
				     ret);
		}
	}
}
