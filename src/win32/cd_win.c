...
static void CDAudio_CloseDoor(void)
{
	...
	if (dwReturn =
	    mciSendCommand(wDeviceID, MCI_SET, MCI_SET_DOOR_CLOSED,
			   (DWORD) NULL))
...
void CDAudio_RandomPlay(void)
{
	...
	for (; i < maxTrack; i++) {
		...
		if (dwReturn) {
			...
			track_bools[i] =
			    (mciStatusParms.dwReturn == MCI_CDA_TRACK_AUDIO);
	...
	do {
		...
		mciPlayParms.dwTo =
		    (mciStatusParms.dwReturn << 8) | remap_track;
		mciPlayParms.dwCallback = (DWORD) cl_hwnd;
		dwReturn =
 ...
 free_end:
	...
	void CDAudio_Stop(void) {
		...
		if (dwReturn =
		    mciSendCommand(wDeviceID, MCI_STOP, 0, (DWORD) NULL))
...
//              Com_Printf("CDAudio_Init: No CD in player.\n");
	...
	void CDAudio_Shutdown(void) {
		...
		CDAudio_Stop();
		if (mciSendCommand
		    (wDeviceID, MCI_CLOSE, MCI_WAIT, (DWORD) NULL))
