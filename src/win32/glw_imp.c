...
qboolean VID_CreateWindow(int width, int height, qboolean fullscreen)
{
	...
	glw_state.hWnd = CreateWindowEx(exstyle,
					...
					stylebits,
					x, y, w, h,
					NULL, NULL, glw_state.hInstance, NULL);
	...
	if (GetVersionEx(&vinfo)) {
		if (vinfo.dwMajorVersion > 4) {
			...
			if (vinfo.dwPlatformId == VER_PLATFORM_WIN32_NT) {
				glw_state.allowdisplaydepthchange = true;
			} else if (vinfo.dwPlatformId ==
				   VER_PLATFORM_WIN32_WINDOWS) {
				if (LOWORD(vinfo.dwBuildNumber) >=
				    OSR2_BUILD_NUMBER) {
		...
		ri.Con_Printf(PRINT_ALL,
			      "GLimp_Init() - GetVersionEx failed\n");
		return false;
	}
...
qboolean GLimp_InitGL(void)
{
	...
	if (glw_state.minidriver) {
		if ((pixelformat =
		     qwglChoosePixelFormat(glw_state.hDC, &pfd)) == 0) {
			ri.Con_Printf(PRINT_ALL,
		...
		DescribePixelFormat(glw_state.hDC, pixelformat, sizeof(pfd),
				    &pfd);
	...
	if (!(pfd.dwFlags & PFD_STEREO) && (stereo->value != 0)) {
		ri.Con_Printf(PRINT_ALL,
			      "...failed to select stereo pixel format\n");
	...
	if ((glw_state.hGLRC = qwglCreateContext(glw_state.hDC)) == 0) {
		ri.Con_Printf(PRINT_ALL,
			      "GLimp_Init() - qwglCreateContext failed\n");
