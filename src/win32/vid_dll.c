...
static void WIN_EnableAltTab(void)
{
	if (s_alttab_disabled) {
		if (s_win95) {
			...
			SystemParametersInfo(SPI_SCREENSAVERRUNNING, 0, &old,
					     0);
		} else {
...
// one update sometimes, so we always check all states and look for events
	...
	case MM_MCINOTIFY:
		{
			LONG CDAudio_MessageHandler(HWND hWnd, UINT uMsg,
						    WPARAM wParam,
						    LPARAM lParam);
			lRet =
...
qboolean VID_LoadRefresh(char *name)
{
	...
	if ((GetRefAPI =
	     (void *)GetProcAddress(reflib_library, "GetRefAPI")) == 0)
...
void VID_CheckChanges(void)
{
	...
	if (vid_xpos->modified || vid_ypos->modified) {
		if (!vid_fullscreen->value)
			VID_UpdateWindowPosAndSize(vid_xpos->value,
						   vid_ypos->value);
