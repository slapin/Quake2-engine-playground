...
static qboolean DS_CreateBuffers(void)
{
	...
	if (DS_OK !=
	    pDS->lpVtbl->SetCooperativeLevel(pDS, cl_hwnd, DSSCL_EXCLUSIVE)) {
		Com_Printf("failed\n");
		FreeSound();
		return false;
	}
...
// sound hardware format
	...
	if (!primary_format_set || !s_primary->value) {
		...
		if (DS_OK !=
		    pDS->lpVtbl->CreateSoundBuffer(pDS, &dsbuf, &pDSBuf,
						   NULL)) {
			Com_Printf("failed\n");
			FreeSound();
			return false;
		...
		if (DS_OK !=
		    pDS->lpVtbl->SetCooperativeLevel(pDS, cl_hwnd,
						     DSSCL_WRITEPRIMARY)) {
			Com_Printf("failed\n");
			FreeSound();
			return false;
...
void SNDDMA_BeginPainting(void)
{
	...
	while ((hresult =
		pDSBuf->lpVtbl->Lock(pDSBuf, 0, gSndBufSize, &pbuf, &locksize,
